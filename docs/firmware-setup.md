...
Inputs use ESP32 internal pull-ups. Lane sensors are captured on falling edges. The start gate is considered **set** when GPIO22 reads HIGH; the race starts after GPIO22 reads LOW continuously for 10 ms, with the start timestamp taken from the first low sample in that stable run.
