...
static constexpr int      WIFI_MODE_PIN            = 21;   // GPIO21 — same as ESP32TimerHAL::WIFI_MODE_PIN
...
void loop() {
    ...
    Serial.println("loop() iteration — should never see this in production!");  // Debug log to verify loop is not running. Remove in production.
    // vTaskDelay(pdMS_TO_TICKS(1000));
    delay(500);  // Yield to tasks. Not strictly necessary since FreeRTOS tasks should be running, but good practice.
}
...
//         Serial.println("CommTask loop iteration");  // Debug log to verify task is running. Remove in production.
